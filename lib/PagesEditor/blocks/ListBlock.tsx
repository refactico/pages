...
export const ListBlock: React.FC<ListBlockProps> = ({ block, onUpdate, readOnly, theme = 'light' }) => {
  ...
  return (
    ...
    >
      ...
      <ListWrapper className={`space-y-1 ${block.listType === 'numbered' ? 'list-decimal' : block.listType === 'bullet' ? 'list-disc' : 'list-none'} ${block.listType !== 'checklist' ? 'pl-6' : ''} ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
        {block.items.map((item, index) => (
          <li key={index} className="group/item flex items-center gap-2">
            ...
            {readOnly ? (
              <span className={block.checkedItems?.[index] ? (isDark ? 'line-through text-slate-500' : 'line-through text-slate-400') : ''}>
                {item}
              </span>
            ) : (
              <>
                <input
                  ...
                  className={`flex-1 outline-none bg-transparent ${
                    block.checkedItems?.[index] ? (isDark ? 'line-through text-slate-500' : 'line-through text-slate-400') : ''
